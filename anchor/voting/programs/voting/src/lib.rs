...
Data Structure:
- Poll("poll", poll_id:u64) -> PollAccount
- Vote(poll_id:u64, candidate:string) -> CandidateAccount

What to Learn:
- Creating Account with seeds
- Updating the states of the account

Deep Analysis on how the contract works:
1. Initialize A Poll, containing information of the polling
2. Initialize A Candidate using the Poll ID, containing information of the candidate

3. When a vote is initialized:
- Check whether the polling is still active or not
- Check whether the polling has started or not
*/
...
#[derive(Accounts)]
#[instruction(poll_id: u64)] // These are the parameters in the function used in the accounts
